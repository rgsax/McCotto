package it.unical.igpe.tinyTanks.core;
...
public class Mondo implements PluginWorld{
	int width;
	int height;
	CarroArmato carro;
	...
	public void spara(CarroArmato c) {
		...
		if(!scatolaInMezzo && c.remainingShots() > 0) {
			bullets.add(new Bullet(c, c.getCannone(), c.getPower()));
			c.decreaseShots();
		}
	}
	...
	@Override
	public ArrayList<CharacterEntity> getPlayerEntities() {
		ArrayList<CharacterEntity> playerEntities = new ArrayList<>();
		for(CharacterEntity player : players.values())
	...
	@Override
	public ArrayList<CharacterEntity> getEnemyEntities() {
		ArrayList<CharacterEntity> enemyEntities = new ArrayList<>();
